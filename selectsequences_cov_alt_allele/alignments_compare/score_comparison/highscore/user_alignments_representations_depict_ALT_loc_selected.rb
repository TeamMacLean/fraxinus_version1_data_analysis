		...
		if entry !~ /^\#/
			...
			if puzzles[bamfilename.to_s].key?(puzzleid.to_s) == true
				puzzles[bamfilename.to_s][puzzleid.to_s].each_key { |key|
					...
					percent = Cigar.sw_score(bwacigar, info[0].upcase, initial_gap, readseq)
					...
					percent2 = Cigar.sw_score(playercigar, info[0].upcase, adjusted2, readseq)
					score1 = percent/readseq.length
					score2 = percent2/readseq.length


#					if percent2 > percent and (percent2 - percent) > 1
					if percent2 > percent
						stats = [percent, percent2, score1, score2].join("\t")
						...
						variants[variant][keyid][key][stats] = alignments
					end
	...
	variants.each_key { |var|
	  variants[var].each_key { |key|
#		if variants[var][key].length > 1 
			file.print "#{var}\t#{key}\n"
			useraln = []
			variants[var][key].each_key { |puzzle|
				variants[var][key][puzzle].each_key { |stats|
					file.print "#{stats}\n"
					print "#{stats}\n"
					alns = variants[var][key][puzzle][stats].split("_")
					file.print "#{alns[0]}"
					useraln.push(alns[1])
				}
			}
