		...
		if entry !~ /^\#/
			...
			if puzzles[bamfilename.to_s].key?(puzzleid.to_s) == true
				puzzles[bamfilename.to_s][puzzleid.to_s].each_key { |key|
					...
					score1 = Cigar.sw_score(bwacigar, info[0].upcase, initial_gap, readseq)
					...
					score2 = Cigar.sw_score(playercigar, info[0].upcase, adjusted2, readseq)
					percent = score1/readseq.length
					percent2 = score2/readseq.length
